...
class UserBatchRepositoryTest {
    ...
    @Test
    @DisplayName("배치 INSERT 테스트")
    @Rollback
    void test_batch_insert() {

        //given
        List<String> lastName = List.of(
                "김","이","박","최","정",
                "강","조","윤","장","임","한",
                "오","서","신","권","황","안","송",
                "류","전","홍","고","문","양","손",
        ...
        String encodedPassword = passwordEncoder.encode("qweasdzxc1!");
        ...
        for (int i = 0; i < 5000000; i += batchSize) {
            List<User> userList = new ArrayList<>();
            for (int j = i; j < i + batchSize && j < 5000000; j++) {

                String email = "person_" + j + "@gmail.com";

                String nickname = lastName.get(random.nextInt(lastName.size()))
                        + firstName.get(random.nextInt(firstName.size()))
                        + firstName.get(random.nextInt(firstName.size()));

                User user = new User(email, nickname, encodedPassword, UserRole.USER);

                userList.add(user);
            }
            //when
            long startTime = System.currentTimeMillis();
            userBatchRepository.batchInsert(userList);
        ...
        //then
        System.out.println("batch Insert 실행됨 : " + totalTime + "(ms)");
    }
}